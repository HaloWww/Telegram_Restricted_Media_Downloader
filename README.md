...
5. 目前机器人支持的命令用法及解释如下表所示：
   ...
   - 机器人支持白名单用户私聊提交`/download`下载任务；普通用户只能查看、取消自己创建的任务，并只会收到自己任务的下载完成通知。
   - `/exit`、`/upload`、`/forward`、`/download_chat`、监听、统计表和设置等管理功能仅当前登录主账号可用。
...
bot_allowed_users: [] # 允许使用机器人下载功能的额外用户ID列表,登录主账号始终允许使用。
...
- `bot_allowed_users`用于设置允许使用机器人下载功能的额外Telegram用户ID列表；登录主账号始终允许使用，无需填写。
- 示例:`bot_allowed_users: [123456789, 987654321]`。
- 主账号也可以在机器人帮助页面点击`⚙️设置` -> `👥机器人白名单`添加或移除用户ID。
...
|  无   | `--debug-log`, `--log-file` | 输出详细DEBUG日志到本地文件 |
...
8. `--debug-log`、`--log-file`参数用法：

   | 使用须知                                                     |
   | ------------------------------------------------------------ |
   | _1.该参数用于输出详细`DEBUG`日志到本地文件,方便测试后排查问题。_ |
   | _2.不指定文件路径时,默认写入当前工作目录下的`TRMD_DEBUG.log`。_ |
   | _3.安装Linux systemd服务时传入该参数,会自动写入服务启动参数。_ |
   ...
   ```bash
   ./TRMD --debug-log /opt/trmd/TRMD_DEBUG.log
   ```

9. Linux systemd服务参数用法：
